...
export function getGreeting(username: string): string {
  ...
  if (hour >= 0 && hour < 5)  return `Still up, ${username}`;
  if (hour >= 5 && hour < 9)  return `Good morning, ${username}`;
  if (hour >= 9 && hour < 12) return `Morning, ${username}`;
  if (hour >= 12 && hour < 14) return `Good afternoon, ${username}`;
  if (hour >= 14 && hour < 17) return `Afternoon, ${username}`;
  if (hour >= 17 && hour < 20) return `Good evening, ${username}`;
  if (hour >= 20 && hour < 23) return `Evening, ${username}`;
